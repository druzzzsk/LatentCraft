...
def main(config_path):
    ...
    print(f"Running gradient ascent (JT-VAE) on {n_seeds} seed molecules...")
    print(f"Steps: {opt_cfg['steps']}, lr: {opt_cfg['lr']}, target: {opt_cfg['target_property']}")

    # charset=None is fine — JTVAEWrapper ignores it
    ...
    if paired_improvements:
        fig1 = plot_property_vs_similarity(
            paired_improvements, paired_similarities,
            title=f"JT-VAE Gradient Ascent: {target_property} improvement vs. Similarity"
        )
        fig1.savefig(f"{out_cfg['dir']}/jtvae_gradient_ascent_scatter.png", dpi=150, bbox_inches="tight")

        fig2 = plot_pareto_front(
            paired_improvements, paired_similarities,
            title=f"Pareto front — JT-VAE Gradient Ascent ({target_property})"
        )
        fig2.savefig(f"{out_cfg['dir']}/jtvae_gradient_ascent_pareto.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/jtvae_gradient_ascent.yaml")
