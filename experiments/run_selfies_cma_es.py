...
def main(config_path):
    ...
    if paired_improvements:
        fig1 = plot_property_vs_similarity(
            paired_improvements,
            paired_similarities,
            title=f"SELFIES CMA-ES: {target_property} improvement vs. Similarity"
        )
        ...
        fig2 = plot_pareto_front(
            paired_improvements,
            paired_similarities,
            title=f"Pareto front — SELFIES CMA-ES ({target_property})"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/selfies_cma_es.yaml")
