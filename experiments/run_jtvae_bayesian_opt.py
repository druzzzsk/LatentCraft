...
def main(config_path):
    ...
    print(f"Running Bayesian Optimization (JT-VAE) on {n_seeds} seeds...")
    ...
    if paired_improvements:
        fig1 = plot_property_vs_similarity(
            paired_improvements, paired_similarities,
            title=f"JT-VAE Bayesian Opt: {target_property} improvement vs. Similarity"
        )
        fig1.savefig(f"{out_cfg['dir']}/jtvae_bayesian_opt_scatter.png", dpi=150, bbox_inches="tight")

        fig2 = plot_pareto_front(
            paired_improvements, paired_similarities,
            title=f"Pareto front — JT-VAE Bayesian Opt ({target_property})"
        )
        fig2.savefig(f"{out_cfg['dir']}/jtvae_bayesian_opt_pareto.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/jtvae_bayesian_opt.yaml")
