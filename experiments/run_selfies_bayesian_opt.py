...
from data.preprocessing_selfies import load_selfies_charset, SelfiesDataset
from models.selfies_vae import SelfiesVAE
from optimizers.bayesian_opt import bayesian_optimization
...
def main(config_path):
    ...
    charset = load_selfies_charset(model_cfg["charset"])
    n_chars = len(charset)
    ...
    dataset = SelfiesDataset(cfg["data"]["path"], max_len=cfg["data"]["max_len"], charset=charset)
    n_total = len(dataset)
    ...
    print(f"Running Bayesian Optimization on {n_seeds} seeds...")
    print(f"n_init: {opt_cfg['n_init']}, n_iter: {opt_cfg['n_iter']}, target: {target_property}")

    optimized_smiles, trajectory = bayesian_optimization(
        vae=vae,
    ...
    if paired_improvements:
        fig1 = plot_property_vs_similarity(
            paired_improvements,
            paired_similarities,
            title=f"SELFIES Bayesian Opt: {target_property} improvement vs. Similarity"
        )
        fig1.savefig(f"{out_cfg['dir']}/bayesian_opt_scatter.png", dpi=150, bbox_inches="tight")

        fig2 = plot_pareto_front(
            paired_improvements,
            paired_similarities,
            title=f"Pareto front — SELFIES Bayesian Opt ({target_property})"
        )
        fig2.savefig(f"{out_cfg['dir']}/bayesian_opt_pareto.png", dpi=150, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/selfies_bayesian_opt.yaml")
