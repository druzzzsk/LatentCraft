...
class JTNNVAE(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, depthT, depthG):
        ...
        self.latent_size = latent_size = latent_size // 2  # Tree and Mol each get half
    ...
    def assm(self, mol_batch, jtmpn_holder, x_mol_vecs, x_tree_mess):
        ...
        for i, mol_tree in enumerate(mol_batch):
            ...
            for node in comp_nodes:
                ...
                if cur_score.data[label].item() >= cur_score.max().item():
                    acc += 1

                label_tensor = create_var(torch.LongTensor([label]))
                all_loss.append(self.assm_loss(cur_score.view(1, -1), label_tensor))
